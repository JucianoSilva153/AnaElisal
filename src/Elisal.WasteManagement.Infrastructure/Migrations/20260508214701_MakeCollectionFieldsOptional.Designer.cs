// <auto-generated />
using System;
using Elisal.WasteManagement.Infrastructure.Persistence;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace Elisal.WasteManagement.Infrastructure.Migrations
{
    [DbContext(typeof(ElisalDbContext))]
    [Migration("20260508214701_MakeCollectionFieldsOptional")]
    partial class MakeCollectionFieldsOptional
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.0")
                .HasAnnotation("Relational:MaxIdentifierLength", 64);

            MySqlModelBuilderExtensions.AutoIncrementColumns(modelBuilder);

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.AuditLog", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Action")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)")
                        .HasColumnName("acao");

                    b.Property<string>("Details")
                        .IsRequired()
                        .HasMaxLength(2000)
                        .HasColumnType("varchar(2000)")
                        .HasColumnName("detalhes");

                    b.Property<string>("TableName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)")
                        .HasColumnName("tabela_afetada");

                    b.Property<DateTime>("Timestamp")
                        .HasColumnType("datetime(6)")
                        .HasColumnName("data_hora");

                    b.Property<int>("UserId")
                        .HasColumnType("int")
                        .HasColumnName("usuario_id");

                    b.HasKey("Id");

                    b.ToTable("LogAuditoria", (string)null);
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.CollectionPoint", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Address")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)")
                        .HasColumnName("endereco");

                    b.Property<double>("Capacity")
                        .HasColumnType("double")
                        .HasColumnName("capacidade");

                    b.Property<double>("CurrentOccupancy")
                        .HasColumnType("double");

                    b.Property<bool>("IsActive")
                        .HasColumnType("tinyint(1)");

                    b.Property<double>("Latitude")
                        .HasColumnType("double")
                        .HasColumnName("latitude");

                    b.Property<double>("Longitude")
                        .HasColumnType("double")
                        .HasColumnName("longitude");

                    b.Property<string>("Municipality")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)")
                        .HasColumnName("municipio");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("varchar(150)")
                        .HasColumnName("nome");

                    b.HasKey("Id");

                    b.ToTable("PontoRecolha", (string)null);
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.CollectionRecord", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<double?>("AmountKg")
                        .HasColumnType("double")
                        .HasColumnName("quantidade_kg");

                    b.Property<int>("CollectionPointId")
                        .HasColumnType("int")
                        .HasColumnName("ponto_recolha_id");

                    b.Property<DateTime>("DateTime")
                        .HasColumnType("datetime(6)")
                        .HasColumnName("data_hora");

                    b.Property<string>("Notes")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)")
                        .HasColumnName("observacoes");

                    b.Property<int>("UserId")
                        .HasColumnType("int")
                        .HasColumnName("usuario_id");

                    b.Property<int?>("WasteTypeId")
                        .HasColumnType("int")
                        .HasColumnName("tipo_residuo_id");

                    b.HasKey("Id");

                    b.HasIndex("CollectionPointId");

                    b.HasIndex("UserId");

                    b.HasIndex("WasteTypeId");

                    b.ToTable("RegistoRecolha", (string)null);
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.CollectionRecordWasteType", b =>
                {
                    b.Property<int>("CollectionRecordId")
                        .HasColumnType("int");

                    b.Property<int>("WasteTypeId")
                        .HasColumnType("int");

                    b.HasKey("CollectionRecordId", "WasteTypeId");

                    b.HasIndex("WasteTypeId");

                    b.ToTable("CollectionRecordWasteTypes");
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.Cooperative", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("AcceptedWasteTypes")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)")
                        .HasColumnName("tipos_residuos_aceites");

                    b.Property<string>("Address")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)")
                        .HasColumnName("endereco");

                    b.Property<string>("Contact")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)")
                        .HasColumnName("contacto");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("varchar(150)")
                        .HasColumnName("email");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("varchar(150)")
                        .HasColumnName("nome");

                    b.HasKey("Id");

                    b.ToTable("Cooperativa", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 999,
                            AcceptedWasteTypes = "Todos",
                            Address = "Elisal Sede",
                            Contact = "N/A",
                            Email = "interno@elisal.co.ao",
                            Name = "Elisal - Reaproveitamento Interno"
                        });
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.OperationalAlert", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<int?>("CollectionPointId")
                        .HasColumnType("int");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<bool>("IsRead")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("Message")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<int?>("RouteId")
                        .HasColumnType("int");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.ToTable("OperationalAlerts");
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.Route", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<int?>("AssignedDriverId")
                        .HasColumnType("int");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)")
                        .HasColumnName("descricao");

                    b.Property<bool>("IsActive")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("varchar(150)")
                        .HasColumnName("nome");

                    b.Property<TimeSpan>("StartTime")
                        .HasColumnType("time(6)")
                        .HasColumnName("hora_inicio");

                    b.Property<double>("TotalDistance")
                        .HasColumnType("double");

                    b.Property<string>("WeekDay")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("dia_semana");

                    b.HasKey("Id");

                    b.HasIndex("AssignedDriverId");

                    b.ToTable("Rota", (string)null);
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.RouteExecution", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("DriverId")
                        .HasColumnType("int");

                    b.Property<DateTime?>("EndTime")
                        .HasColumnType("datetime(6)");

                    b.Property<int>("RouteId")
                        .HasColumnType("int");

                    b.Property<DateTime>("StartTime")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.HasIndex("DriverId");

                    b.HasIndex("RouteId");

                    b.ToTable("RouteExecutions");
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.RoutePoint", b =>
                {
                    b.Property<int>("RouteId")
                        .HasColumnType("int")
                        .HasColumnName("rota_id");

                    b.Property<int>("CollectionPointId")
                        .HasColumnType("int")
                        .HasColumnName("ponto_recolha_id");

                    b.Property<int>("SequenceOrder")
                        .HasColumnType("int")
                        .HasColumnName("ordem_sequencia");

                    b.HasKey("RouteId", "CollectionPointId");

                    b.HasIndex("CollectionPointId");

                    b.ToTable("RotaPonto", (string)null);
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.RoutePointExecutionStatus", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("CollectionPointId")
                        .HasColumnType("int");

                    b.Property<DateTime?>("CompletedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<bool>("IsCompleted")
                        .HasColumnType("tinyint(1)");

                    b.Property<bool>("IsSkipped")
                        .HasColumnType("tinyint(1)");

                    b.Property<int>("RouteExecutionId")
                        .HasColumnType("int");

                    b.Property<string>("SkipReason")
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.HasIndex("CollectionPointId");

                    b.HasIndex("RouteExecutionId");

                    b.ToTable("RoutePointExecutionStatuses");
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.SortingBatch", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime?>("EndDateTime")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("Notes")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.Property<int>("OperatorUserId")
                        .HasColumnType("int");

                    b.Property<DateTime>("StartDateTime")
                        .HasColumnType("datetime(6)");

                    b.Property<int>("Status")
                        .HasColumnType("int");

                    b.Property<int>("WasteReceptionId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("OperatorUserId");

                    b.HasIndex("StartDateTime");

                    b.HasIndex("Status");

                    b.HasIndex("WasteReceptionId");

                    b.ToTable("SortingBatches");
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.SortingBatchOutput", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("DestinationType")
                        .HasColumnType("int");

                    b.Property<string>("Notes")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.Property<int>("QualityGrade")
                        .HasColumnType("int");

                    b.Property<int>("SortingBatchId")
                        .HasColumnType("int");

                    b.Property<int>("WasteTypeId")
                        .HasColumnType("int");

                    b.Property<double>("WeightKg")
                        .HasColumnType("double");

                    b.HasKey("Id");

                    b.HasIndex("SortingBatchId");

                    b.HasIndex("WasteTypeId");

                    b.ToTable("SortingBatchOutputs");
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.SortingCenter", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Address")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.Property<double>("CapacityTonsPerDay")
                        .HasColumnType("double");

                    b.Property<string>("Contact")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<bool>("IsActive")
                        .HasColumnType("tinyint(1)");

                    b.Property<double>("Latitude")
                        .HasColumnType("double");

                    b.Property<double>("Longitude")
                        .HasColumnType("double");

                    b.Property<string>("Municipality")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("varchar(150)");

                    b.HasKey("Id");

                    b.HasIndex("Name");

                    b.ToTable("SortingCenters");
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.Transaction", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<double>("AmountKg")
                        .HasColumnType("double")
                        .HasColumnName("quantidade_kg");

                    b.Property<int>("CooperativeId")
                        .HasColumnType("int")
                        .HasColumnName("cooperativa_id");

                    b.Property<DateTime>("Date")
                        .HasColumnType("datetime(6)")
                        .HasColumnName("data");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)")
                        .HasColumnName("status");

                    b.Property<decimal>("Value")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("valor");

                    b.Property<int>("WasteTypeId")
                        .HasColumnType("int")
                        .HasColumnName("tipo_residuo_id");

                    b.HasKey("Id");

                    b.HasIndex("CooperativeId");

                    b.HasIndex("WasteTypeId");

                    b.ToTable("Transacao", (string)null);
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.User", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime(6)")
                        .HasColumnName("data_cadastro");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("varchar(150)")
                        .HasColumnName("email");

                    b.Property<bool>("IsActive")
                        .HasColumnType("tinyint(1)")
                        .HasColumnName("ativo");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("varchar(150)")
                        .HasColumnName("nome");

                    b.Property<string>("PasswordHash")
                        .IsRequired()
                        .HasColumnType("longtext")
                        .HasColumnName("senha_hash");

                    b.Property<string>("Role")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)")
                        .HasColumnName("perfil");

                    b.HasKey("Id");

                    b.HasIndex("Email")
                        .IsUnique();

                    b.ToTable("Usuario", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Email = "root@elisal.ao",
                            IsActive = true,
                            Name = "Root Admin",
                            PasswordHash = "$2a$12$kRntuZvjc1inEbVrN//K/uVjXSiPDTuX8eJtmIKa9fc9rlFs483/a",
                            Role = "Admin"
                        });
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.UserToken", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime>("ExpiryDate")
                        .HasColumnType("datetime(6)");

                    b.Property<bool>("IsRevoked")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("RefreshToken")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("Token")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<int>("UserId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("UserTokens");
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.WasteReception", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("DateTime")
                        .HasColumnType("datetime(6)");

                    b.Property<double>("GrossWeightKg")
                        .HasColumnType("double");

                    b.Property<bool>("IsSorted")
                        .HasColumnType("tinyint(1)");

                    b.Property<double>("NetWeightKgStored")
                        .HasColumnType("double");

                    b.Property<string>("Notes")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.Property<int>("ReceivedByUserId")
                        .HasColumnType("int");

                    b.Property<int?>("RouteExecutionId")
                        .HasColumnType("int");

                    b.Property<int>("SortingCenterId")
                        .HasColumnType("int");

                    b.Property<double>("TareWeightKg")
                        .HasColumnType("double");

                    b.HasKey("Id");

                    b.HasIndex("DateTime");

                    b.HasIndex("IsSorted");

                    b.HasIndex("ReceivedByUserId");

                    b.HasIndex("RouteExecutionId");

                    b.HasIndex("SortingCenterId");

                    b.ToTable("WasteReceptions");
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.WasteType", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("ColorCode")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("cor_identificacao");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)")
                        .HasColumnName("descricao");

                    b.Property<bool>("IsRecyclable")
                        .HasColumnType("tinyint(1)")
                        .HasColumnName("reciclavel");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)")
                        .HasColumnName("nome");

                    b.HasKey("Id");

                    b.ToTable("TipoResiduo", (string)null);
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.CollectionRecord", b =>
                {
                    b.HasOne("Elisal.WasteManagement.Domain.Entities.CollectionPoint", "CollectionPoint")
                        .WithMany()
                        .HasForeignKey("CollectionPointId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Elisal.WasteManagement.Domain.Entities.User", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Elisal.WasteManagement.Domain.Entities.WasteType", "WasteType")
                        .WithMany()
                        .HasForeignKey("WasteTypeId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("CollectionPoint");

                    b.Navigation("User");

                    b.Navigation("WasteType");
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.CollectionRecordWasteType", b =>
                {
                    b.HasOne("Elisal.WasteManagement.Domain.Entities.CollectionRecord", "CollectionRecord")
                        .WithMany("RecordWasteTypes")
                        .HasForeignKey("CollectionRecordId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Elisal.WasteManagement.Domain.Entities.WasteType", "WasteType")
                        .WithMany()
                        .HasForeignKey("WasteTypeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("CollectionRecord");

                    b.Navigation("WasteType");
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.Route", b =>
                {
                    b.HasOne("Elisal.WasteManagement.Domain.Entities.User", "AssignedDriver")
                        .WithMany()
                        .HasForeignKey("AssignedDriverId");

                    b.Navigation("AssignedDriver");
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.RouteExecution", b =>
                {
                    b.HasOne("Elisal.WasteManagement.Domain.Entities.User", "Driver")
                        .WithMany()
                        .HasForeignKey("DriverId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Elisal.WasteManagement.Domain.Entities.Route", "Route")
                        .WithMany()
                        .HasForeignKey("RouteId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Driver");

                    b.Navigation("Route");
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.RoutePoint", b =>
                {
                    b.HasOne("Elisal.WasteManagement.Domain.Entities.CollectionPoint", "CollectionPoint")
                        .WithMany()
                        .HasForeignKey("CollectionPointId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Elisal.WasteManagement.Domain.Entities.Route", "Route")
                        .WithMany("RoutePoints")
                        .HasForeignKey("RouteId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("CollectionPoint");

                    b.Navigation("Route");
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.RoutePointExecutionStatus", b =>
                {
                    b.HasOne("Elisal.WasteManagement.Domain.Entities.CollectionPoint", "CollectionPoint")
                        .WithMany()
                        .HasForeignKey("CollectionPointId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Elisal.WasteManagement.Domain.Entities.RouteExecution", "RouteExecution")
                        .WithMany("PointStatuses")
                        .HasForeignKey("RouteExecutionId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("CollectionPoint");

                    b.Navigation("RouteExecution");
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.SortingBatch", b =>
                {
                    b.HasOne("Elisal.WasteManagement.Domain.Entities.User", "Operator")
                        .WithMany()
                        .HasForeignKey("OperatorUserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Elisal.WasteManagement.Domain.Entities.WasteReception", "WasteReception")
                        .WithMany("SortingBatches")
                        .HasForeignKey("WasteReceptionId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Operator");

                    b.Navigation("WasteReception");
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.SortingBatchOutput", b =>
                {
                    b.HasOne("Elisal.WasteManagement.Domain.Entities.SortingBatch", "SortingBatch")
                        .WithMany("Outputs")
                        .HasForeignKey("SortingBatchId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Elisal.WasteManagement.Domain.Entities.WasteType", "WasteType")
                        .WithMany()
                        .HasForeignKey("WasteTypeId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("SortingBatch");

                    b.Navigation("WasteType");
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.Transaction", b =>
                {
                    b.HasOne("Elisal.WasteManagement.Domain.Entities.Cooperative", "Cooperative")
                        .WithMany()
                        .HasForeignKey("CooperativeId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Elisal.WasteManagement.Domain.Entities.WasteType", "WasteType")
                        .WithMany()
                        .HasForeignKey("WasteTypeId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Cooperative");

                    b.Navigation("WasteType");
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.UserToken", b =>
                {
                    b.HasOne("Elisal.WasteManagement.Domain.Entities.User", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.WasteReception", b =>
                {
                    b.HasOne("Elisal.WasteManagement.Domain.Entities.User", "ReceivedByUser")
                        .WithMany()
                        .HasForeignKey("ReceivedByUserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Elisal.WasteManagement.Domain.Entities.RouteExecution", "RouteExecution")
                        .WithMany()
                        .HasForeignKey("RouteExecutionId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("Elisal.WasteManagement.Domain.Entities.SortingCenter", "SortingCenter")
                        .WithMany()
                        .HasForeignKey("SortingCenterId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("ReceivedByUser");

                    b.Navigation("RouteExecution");

                    b.Navigation("SortingCenter");
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.CollectionRecord", b =>
                {
                    b.Navigation("RecordWasteTypes");
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.Route", b =>
                {
                    b.Navigation("RoutePoints");
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.RouteExecution", b =>
                {
                    b.Navigation("PointStatuses");
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.SortingBatch", b =>
                {
                    b.Navigation("Outputs");
                });

            modelBuilder.Entity("Elisal.WasteManagement.Domain.Entities.WasteReception", b =>
                {
                    b.Navigation("SortingBatches");
                });
#pragma warning restore 612, 618
        }
    }
}
